...
- Side effects: 없음
- Related API: 클라이언트 hydration 후 `GET /api/community/categories`, `GET /api/posts`; 사용 가능한 token이 있으면 `GET /api/users/me`, `GET /api/onboarding/keywords`
...
- Steps:
  ...
  5. 사용자가 추천 키워드 토글 또는 직접 입력으로 첫 관심 키워드를 선택한다.
...
- Actor: 사용자
- Entry point: 헤더 `커뮤니티`
- Preconditions: 앱이 클라이언트에서 hydration 완료, 백엔드 게시판 API 접근 가능
- Steps:
  ...
  4. 전체, 정치, 경제, 사회, IT/과학, 세계, 스포츠, 연예 게시판 필터 중 하나를 선택한다.
...
- Permission behavior: 게시글 목록은 선택 인증이다. token이 있으면 `isMine` 계산을 위해 Bearer token과 cookie를 함께 보낸다.
...
- Steps:
  ...
  2. 로그인 사용자가 아니면 Google OAuth 로그인을 시작하고 메시지를 표시한다.
...
- Permission behavior: 상세/댓글 조회는 선택 인증, 댓글 작성과 좋아요 변경은 Bearer token과 `accessToken` cookie 필요
