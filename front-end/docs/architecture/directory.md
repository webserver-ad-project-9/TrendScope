...
| `front-end/trend-scope-front/.env.example` | file | 환경변수 템플릿 | Configuration | 실제 secret 없이 필요한 env key만 문서화 |
...
| `front-end/trend-scope-front/src/config/environment.ts` | file | 환경변수 access module | Configuration | 백엔드 Base URL, token storage/cookie key 설정 |
...
- `.env.local`은 Git에 커밋하지 않고, 새 환경변수는 `.env.example`에 secret 없이 추가한다.
