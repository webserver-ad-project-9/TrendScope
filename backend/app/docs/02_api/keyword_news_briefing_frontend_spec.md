...
로그인한 사용자의 온보딩 선호 키워드를 기준으로 접속일 당일 뉴스를 수집하고, 키워드별 브리핑을 반환한다.
...
현재 로그인한 사용자의 활성 온보딩 키워드를 조회한 뒤, 오늘 날짜 뉴스 최대 50개를 키워드별로 최대한 균등하게 수집하고 그룹화한다.
...
3. 서버 기준 날짜 `Asia/Seoul`의 당일 뉴스만 조회한다.
4. 전체 기사 수는 최대 50개다.
5. 키워드가 여러 개면 50개를 키워드 수로 나누어 최대한 균등하게 배정한다.
6. 나누어떨어지지 않는 나머지는 앞쪽 키워드부터 1개씩 추가 배정한다.
7. 특정 키워드의 기사가 부족하면 가능한 만큼만 반환한다.
8. 응답 생성 시 URL 또는 제목이 이미 포함된 기사는 중복 제거한다.
9. 요약은 기사 본문이 아니라 해당 키워드에 포함된 기사 제목 목록만 기반으로 생성한다.
10. 요약 실패 또는 기사 없음이면 fallback 문구를 반환한다.
...
```json
{
  ...
  "data": {
    ...
    "totalCollectedCount": 50,
    "summaries": [
      {
        "keyword": "스포츠",
        "collectedCount": 10,
        "summary": "오늘 스포츠 분야에서는 국내 프로야구 순위 경쟁과 해외 축구 이적설이 주요 흐름으로 나타났다. 주요 선수들의 부상 복귀와 경기 결과가 함께 주목받았다.",
        "articles": [
          {
            "title": "프로야구 순위 경쟁 심화",
...
| `totalCollectedCount` | number | N | 전체 수집 기사 수. 최대 50 |
...
특정 키워드에 당일 기사가 없으면 해당 키워드는 `collectedCount: 0`, `articles: []`, fallback `summary`를 반환할 수 있다.
